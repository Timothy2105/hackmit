...
def main():
    parser = argparse.ArgumentParser(description="Run VGGT inference on images from Supabase")
    parser.add_argument(
        "--bucket-path", 
    ...
    # Download images from specified bucket path
    images_dir, image_paths = download_images_from_supabase(
        supabase_client=supabase_client,
        bucket_name=SUPABASE_BUCKET,
        user_folder=args.bucket_path
    )
    
    # Run VGGT and create PLY
    if image_paths:
        run_vggt_and_create_ply(image_paths, images_dir.parent)
    else:
